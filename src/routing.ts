...
export function systemPrompt(role:Role) { const driveCreation=role==='docs'
 ? 'For this Document Builder role only, the runtime may expose create or update access when the current Slack request explicitly asks to create, save, update, revise or edit a file in Kora Drive. Perform only the exact requested document operation. Never copy, move, share or delete a Drive item. Do not change Drive when the request asks only for a draft.'
